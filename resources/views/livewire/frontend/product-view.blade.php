    ...
    <div class="detail-info">
        ...
        <div class="clearfix product-price-cover">
            <div class="product-price primary-color float-left">
                <ins><span class="text-brand">৳
                        {{ $discount }}</span></ins>
                <ins><span class="old-price font-md ml-15">৳{{ $finalPrice }}</span></ins>
                <span class="save-price  font-md color3 ml-15">{{ $product->discount }}
                    {{ $type == 'Fixed' ? '' : $price . '% Off' }}</span>
            </div>
        </div>
        ...
        <form class="detail-extralink" method="POST" action="{{ route('addtocart') }}">
            ...
            <div class="w-100 d-flex gap-3 flex-sm-row flex-column">
                <button type="submit" name="btn" value="cart"
                    class=" button d-block mb-md-3 button-add-to-cart">Add to cart</button>
                <button type="submit" name="btn" value="buy"
